# Australian Home Collective publishing foundation

This directory is the version-controlled source of truth for Australian Home Collective's publishing standards, practical knowledge, risk controls and current roadmaps. The set is deliberately small: completed project plans and duplicated public-policy drafts do not remain as operating standards.

## Canonical documents

### Governance

- [Publishing Standards Manual](governance/publishing-standards-manual.md)
- [Editorial Charter](governance/editorial-charter.md)
- [Editorial Decision Framework](governance/editorial-decision-framework.md)

### Editorial

- [Editorial Style Guide](editorial/editorial-style-guide.md)
- [Brand Style Guide](editorial/brand-style-guide.md)
- [Recommendation Methodology](editorial/recommendation-methodology.md)
- [Corrections and Content Review Policy](editorial/corrections-content-review-policy.md)

### Knowledge

- [Retail Knowledge Base](knowledge/retail-knowledge-base.md)

### Tools

- [Planning Tool Standard](tools/planning-tool-standard.md)

### Legal and risk

- [Legal and risk register](legal/README.md)
- [Tool Disclaimer Standard](legal/tool-disclaimer-standard.md)

### Roadmap

- [Publication Roadmap](roadmap/publication-roadmap.md)
- [Planning Centre Roadmap](roadmap/planning-centre-roadmap.md)
- [Commission Factory Readiness](roadmap/commission-factory-readiness.md)
- [Phase 1 Audit Report](roadmap/phase-1-audit-report.md)

### Blueprints

- [Australian Home Collective Publication Intelligence Studio](blueprints/ahc-publication-intelligence-studio.md)

## Supporting audit artefacts

The `visual/` folder contains the current image inventory, assessments, contact sheets and replacement manifest. These are generated or evidence records rather than publishing policies.

There is no operations folder. Production deployment remains documented in the repository README and is not duplicated here.

## Document control

- Owner: Australian Home Collective
- Foundation date: 18 July 2026
- Canonical-set review: 24 July 2026
- Review approach: substantive reviews only; version dates are not advanced automatically
